# 人脈管理系統 — 專案備忘

## 老闆資訊
- GitHub: kuoyo20
- 系統帳號: kuoyo (admin)

## 團隊權限體系

### S級權限（admin）— 最高管理權限
- 溫蒂 wendy.tseng@miraclex.com.tw（大特助，總管所有公司）

### A級權限（editor）— 編輯權限
- S級全部 + 以下：
- 小美 melody@bettermilk.com.tw（鮮乳坊特助）
- Huda huda.huang@miraclex.com.tw（專案型特助）
- Anna anna.chen@miaolin.com.tw（苗林行特助）

### B級權限（editor/viewer）— 全開
- A級全部 + 以下：
- 連連 booking@miraclex.com.tw（私人秘書）

### 預設密碼
- MX統編：66446236

## 帳號對照表

| 帳號 | 姓名 | Email | 角色 | 公司 |
|------|------|-------|------|------|
| kuoyo | 老闆 | kuoyo@miraclex.com.tw | admin | - |
| wendy | 溫蒂 | wendy.tseng@miraclex.com.tw | admin | MiracleX（總管） |
| melody | 小美 | melody@bettermilk.com.tw | admin | 鮮乳坊 |
| huda | Huda | huda.huang@miraclex.com.tw | editor | MiracleX |
| anna | Anna | anna.chen@miaolin.com.tw | editor | 苗林行 |
| lianlian | 連連 | booking@miraclex.com.tw | editor | MiracleX |

## 技術備忘
- 部署：Render.com (Docker) — https://forfunfun-cms.onrender.com
- Branch: claude/contact-management-system-W1wLH
- 資料庫：SQLite（/app/data/contacts.db）
- OCR：tesseract（中/英/日/韓）
- PWA：支援加入主畫面
